                        ...
                        for cycle in 0..341 {
                            if 1 <= cycle && cycle <= 256 && line <= 239 {
                                ...
                                if (cycle % 8) == 0 {
                                    // load tile bit
                                    let flag = (cycle == 256);
                                    // let (fine_y_scroll, name_table_num, y_panel_pos, x_panel_pos) = self.v_ram_address_register
                                    //     .lock()
                                    //     .unwrap()
                                    ...
                                    self.attr_value_register = self.v_ram.lock().unwrap().fetch8(
                                        ((cycle as u16 / 4) +
                                             (line as u16 / 2) * 8),
                                    );
                                    // if line >= 112 && line <= 120{
